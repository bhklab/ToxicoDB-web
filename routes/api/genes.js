...
const getGenes = function (request, response) {
    knex.select('id', 'name', 'symbol', 'ensembl_tid', 'entrez_gid', 'transcript_name', 'full_name')
        .from('genes')
        .leftJoin(
            'gene_annotations',
            'genes.id',
...
const getGeneAnalysis = (request, response) => {
    ...
    knex.distinct('dose', 'time', 'fdr', 'fold_change', 'p_value', 'drugs.id AS drug_id', 'drugs.name AS drug_name', 'datasets.name AS dataset_name')
